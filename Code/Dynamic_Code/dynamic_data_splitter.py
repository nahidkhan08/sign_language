...
# Make sure to provide the correct path for your dynamic data
DYNAMIC_SOURCE_DIR = "Dynamic_Data" 

# Define the main output directory where the split data will be saved
BASE_OUTPUT_DIR = "Dynamic_Data_Split"
...
if __name__ == "__main__":
    print("Starting the dynamic data splitting process...")
    
    # --- Process all directories inside the DYNAMIC source folder ---
    if os.path.exists(DYNAMIC_SOURCE_DIR):
        print(f"\nProcessing dynamic data from '{DYNAMIC_SOURCE_DIR}'...")
        for class_name in os.listdir(DYNAMIC_SOURCE_DIR):
            class_path = os.path.join(DYNAMIC_SOURCE_DIR, class_name)
            if os.path.isdir(class_path):
                split_class_data(class_path, class_name, BASE_OUTPUT_DIR)
    else:
        print(f"Warning: Directory '{DYNAMIC_SOURCE_DIR}' not found. Skipping.")
        
    print("\nDynamic data splitting process completed successfully!")
